...
import {
  ...
  Users, BarChart3, Link2, Tag, Server, ArrowLeft, KeyRound,
} from 'lucide-react';
import { useToast } from '@/components/Toast';
import {
  listSoftware, createSoftware, updateSoftware, deleteSoftware,
...
function SoftwareModal({
  mode, form, setForm, onSubmit, onClose, isPending,
}: {
  mode: ModalMode;
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSubmit(); }} className="max-h-[60vh] overflow-auto p-6">
          ...
          {section === 'ops' && (
            <div className="space-y-4">
              ...
              <StringListEditor label="Dependencies" value={form.dependencies ?? []}
                onChange={(v) => setForm({ ...form, dependencies: v })} placeholder="software-slug" />
            </div>
          )}
            ...
            <div className="mb-3">
              ...
              <div className="flex flex-wrap gap-1">
                {software.dependencies!.map((d) => (
                  <span key={d} className="rounded bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700">{d}</span>
                ))}
              </div>
            </div>
          )}
