...
"""Triage Agent microservice - FastAPI app with A2A endpoints."""
...
agent = TriageAgent(api_base=API_BASE, api_key=API_KEY, model=MODEL)

card = AgentCard(
    name="RootCauseway Triage Agent",
    description="Assesses alert severity, categorizes issues, and identifies affected components",
    url=AGENT_URL,
    version="0.1.0",
    ...
    skills=[
        AgentSkill(
            id="triage",
            name="Alert Triage",
            description="Analyze an alert to determine severity, category, affected components, and provide a summary",
        )
    ],
)

app = FastAPI(title="RootCauseway Triage Agent", version="0.1.0")
